const AudioManager = {
  ...
  /**
   * 위치 기반 반향 효과를 생성하고 결과를 캐싱합니다.
   * @param {THREE.Vector3} position - 플레이어의 현재 위치
   * @return {Object|null} - 반향 효과 객체 또는 null
   */
  ...
  playSpatialGunshot(soundName, position) {
    ...
    // 기본 총소리가 필요하면 아래 코드를 활성화할 수 있습니다
    /*
    // 명확하게 분리하여 기본 총소리 재생 (더 큰 소리로 확실하게)
    const mainShot = this.sounds[soundName] ? this.sounds[soundName]({
      type: 'powerful', // 강력한 소리로 설정
      volume: 1.0       // 최대 볼륨
    }) : null;
    */

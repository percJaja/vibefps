...
const BGMManager = {
  ...
  handlePageVisible() {
    ...
    if (this.audioEnabled && this.currentTrack) {
      // 절차적 트랙인 경우 오디오 컨텍스트 재개
      if (this.currentTrack.nodes && this.currentTrack.nodes.audioCtx) {
        if (this.currentTrack.nodes.audioCtx.state === 'suspended') {
          this.currentTrack.nodes.audioCtx.resume();
        }
      }
  ...
  createFallbackTrack(trackName) {
    ...
    const fallbackTrack = {
      ...
      loop: true,
      volume: 0,
      _trackId: trackName, // 트랙 ID 추가 (추가됨)
      ...
      pause() {
        if (this.nodes && !this.paused) {
          this.paused = true;
          this._intentionallyPaused = true; // 의도적 일시 중지 플래그 추가 (추가됨)
      ...
      // stop 메서드 추가 (추가됨) - 완전히 정지하고 리소스 해제
      stop() {
        this.pause();
        if (this.nodes) {
          try {
            if (this.nodes.oscillator) {
              this.nodes.oscillator.stop();
            }
            if (this.nodes.lfo) {
              this.nodes.lfo.stop();
            }
          } catch (e) {
            // 이미 중지된 경우 오류가 발생할 수 있음, 무시
          }
          // 기존 노드 연결 해제
  ...
  play(trackName, fadeInDuration = 2.0) {
    ...
    // 트랙 재생 전에 currentTime 재설정 (새로 추가됨)
    ...
    // 모바일 환경과 PC 환경에서 다른 페이드인 처리 (추가됨)
    if (this.isMobile) {
      ...
      } else if (newTrack.nodes && newTrack.nodes.gainNode) {
        newTrack.nodes.gainNode.gain.value = targetVolume * 0.2;
      }
  ...
  /**
   * 트랙을 완전히 중지하고 리소스 정리 (추가됨)
   * @param {HTMLAudioElement|Object} track - 중지할 트랙
   */
  stopTrack(track) {
    if (!track) return;
  ...
  pause() {
    if (this.currentTrack) {
      this.currentTrack._intentionallyPaused = true; // 의도적 일시 중지 플래그 추가 (추가됨)
  ...
  resume() {
    if (this.currentTrack && this.currentTrack.paused) {
      delete this.currentTrack._intentionallyPaused; // 의도적 일시 중지 플래그 제거 (추가됨)
  ...
  stopAll() {
    ...
    for (const trackName in this.tracks) {
      ...
      if (track) {
        this.stopTrack(track); // 변경: stopTrack 메서드 사용
      }
    }
  ...
  fadeIn(track, duration = 2.0) {
    ...
    if (typeof track.volume !== 'undefined') {
      ...
      // 절차적 트랙 처리
      const gainNode = track.nodes.gainNode;
      gainNode.gain.value = 0;
      
      // 현재 오디오 컨텍스트 시간
      const now = track.nodes.audioCtx.currentTime;
      
      // 리니어 램프로 볼륨 증가 (Web Audio API 사용)
  ...
  fadeOut(track, duration = 2.0) {
    ...
    if (typeof track.volume !== 'undefined') {
      ...
      // 트랙별 타이머 ID 생성 (로컬 변수에서 저장 방식으로 변경)
      const timerId = setInterval(() => {
        // 볼륨 감소
      ...
      // Web Audio API의 linearRampToValueAtTime 사용
